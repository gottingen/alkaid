...
namespace alkaid {
    ...
    namespace internals {
        ...
        template<typename T>
        constexpr long double pow10(const T &n) noexcept {
        ...
        /** Compute 10 to the power of n */
        template<>
        constexpr long double pow10(const unsigned &n) noexcept {
            long double multiplicand = n > 0 ? 10 : 0.1,
                    ret = 1;
         ...
         *  it fits in
         */
        constexpr DataType _determine_integral_type(const long double &number) noexcept {
         ...
         *                  get stored
         */
        constexpr DataType data_type(std::string_view in, long double *const out) {
            // Empty string --> NULL
            if (in.size() == 0)
