...
# include <cstddef>
using mmap_source = alkaid::basic_mmap_source<std::byte>;
...
#define CHECK_INVALID_MMAP(m) do { \
        ...
        alkaid::mmap_source m;

        // See if mapping an invalid file results in an error.
        m = alkaid::make_mmap_source("garbage-that-hopefully-doesnt-exist", 0, 0, error);
        CHECK_INVALID_MMAP(m);

        // Empty path?
        m = alkaid::make_mmap_source(static_cast<const char*>(0), 0, 0, error);
        CHECK_INVALID_MMAP(m);
        m = alkaid::make_mmap_source(std::string(), 0, 0, error);
        CHECK_INVALID_MMAP(m);

        // Invalid handle?
        m = alkaid::make_mmap_source(alkaid::invalid_handle, 0, 0, error);
        CHECK_INVALID_MMAP(m);

        // Invalid offset?
        m = alkaid::make_mmap_source(path, 100 * buffer.size(), buffer.size(), error);
        CHECK_INVALID_MMAP(m);
    }

    // Make sure these compile.
    {
        alkaid::ummap_source _1;
        alkaid::shared_ummap_source _2;
        // Make sure shared_mmap mapping compiles as all testing was done on
        // normal mmaps.
        alkaid::shared_mmap_source _3(path, 0, alkaid::map_entire_file);
        auto _4 = alkaid::make_mmap_source(path, error);
        auto _5 = alkaid::make_mmap<alkaid::shared_mmap_source>(path, 0, alkaid::map_entire_file, error);
#ifdef _WIN32
        const wchar_t* wpath1 = L"dasfsf";
        auto _6 = alkaid::make_mmap_source(wpath1, error);
        alkaid::mmap_source _7;
        _7.map(wpath1, error);
        const std::wstring wpath2 = wpath1;
        auto _8 = alkaid::make_mmap_source(wpath2, error);
        alkaid::mmap_source _9;
        _9.map(wpath1, error);
#else
        const int fd = open(path, O_RDONLY);
        alkaid::mmap_source _fdmmap(fd, 0, alkaid::map_entire_file);
        _fdmmap.unmap();
                    ...
                    const size_t offset, std::error_code& error)
{
    ...
    alkaid::mmap_source file_view = alkaid::make_mmap_source(
            path, offset, alkaid::map_entire_file, error);
    if(error) { return; }
    ...
    alkaid::shared_mmap_source shared_file_view(std::move(file_view));
    assert(!file_view.is_open());
